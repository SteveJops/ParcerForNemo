    ...
    for profile_inn in tqdm(index_inn):
        ...
        try:
            if profile_inn:
                ...
                an_user = a_session.query(Profile).filter_by(inn=profile_inn).all()

                if an_user:
                    # if user exist than adds all the info that we need to new_data dict
                    ...
                    new_data['parent_pib'] = str(user_pib[index]).split("\n")[0]
                ...
                if len(relative_profile_inn) > 9:
                    ...
                    relative_user = a_session.query(Profile).filter_by(inn=relative_profile_inn).all()
                    if relative_user:
                        try:
                            # if user exist than adds all the info that we need to new_data dict
                            ...
                            new_data['person_pib'] = str(relative_connection_pib_profile[index]).split("\n")[0]
                            new_data['person_path'] = relative_connection_path[index]
                        except Exception:
                            pass

                    else:
                        # if user doesn`t exist than adds all the info that we need to new_data dict and adds to our db
                        new_data['person_inn'] = str(relative_profile_inn)
                        new_data['person_pib'] = str(relative_connection_pib_profile[index]).split("\n")[0]
                        new_data['person_path'] = relative_connection_path[index]
                        try:
                            # splitting pib to divided names ( name, last_name, mid_name)
                            pib_split_by = new_data['person_pib'].strip().split(' ')
                            print(f"PIB else: {pib_split_by}")
                            (last_name, first_name, middle_name) = pib_split_by
                            # checking user`s gender
                            # if middle_name.endswith('ович') or  middle_name.endswith('ОВИЧ'):
                            # elif middle_name.endswith('вна') or  middle_name.endswith('ВНА'):
                            # adding user
                            new_relative_user = Profile(person_type_id=15, inn=str(new_data['person_inn']), last_name=last_name, first_name=first_name, middle_name=middle_name, last_update=datetime.now())
                            a_session.add(new_relative_user)
                            a_session.commit()
                            exist_user = a_session.query(Profile).filter_by(inn=str(new_data['person_inn'])).all()
                            exist_user = str(exist_user[0]).split(" ")
                            exist_user = int(exist_user[1])

                            user_path = Link(person_id=exist_user, link=str(new_data['person_path']), site_id=5)
            ...
            index += 1
            df1 = df1.append(pd.DataFrame(dict_data_with_not_find, index=range(len(df1), len(df1) + len(dict_data_with_not_find))))
            df = df.append(pd.DataFrame(new_data, index=range(len(df), len(df) + len(new_data))))
        except Exception as e:
            print(f"error: {e}")
